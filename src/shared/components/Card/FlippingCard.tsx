...
import { flipCard } from '@/redux/features/flipCard/flipCardSlice';
...
const FlippingCard: React.FC<FlipCardProps> = ({ className, card }: FlipCardProps) => {
  ...
  return (
    <div
      className="group max-w-[250px] max-h-[350px] h-80 w-49 [perspective:1000px] overflow-hidden"
      onClick={() => {
      ...
      >
        ...
        <div className="absolute h-full w-full rounded-xl bg-black/80  text-center text-slate-200 [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <img
            ...
            alt={`${card.alt} | ${card.tempID} | isDone: ${card.isDone}`}
          />
        </div>
      </div>
    </div>
